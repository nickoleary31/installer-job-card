...
export default function CustomerNewPage() {
  ...
  const handleSave = async () => {
    if (!isAdminForCompany) {
      setSaveError("Only admins can create new customers/sites.");
      return;
    }
    ...
    <main className="min-h-screen bg-slate-50 py-6">
      <div className="mx-auto max-w-3xl space-y-4 px-4 sm:px-5">
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-[0_1px_3px_rgba(15,23,42,0.06)] sm:p-6">
          {authLoading ? (
            <p className="text-sm text-gray-600">Loading access...</p>
          ) : !isAdminForCompany ? (
            <p className="text-sm font-semibold text-amber-800">Only admins can create new customers/sites.</p>
          ) : (
            <>
              <CustomerEditorForm
                form={form}
