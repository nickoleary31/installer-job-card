...
/** Mirrors lib/local-photo.ts's LOCAL_PHOTO_URI_SCHEME on feature/mobile-shell (that module is native-only and not on main); a still-local photo reference must never be treated as a storage path. */
const LOCAL_PHOTO_URI_SCHEME = "local-photo://";
